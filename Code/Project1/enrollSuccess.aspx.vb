...
#End Region
        ...
        Private Function VerifyAddress() As Boolean
            ...
            If ddlCountry.SelectedValue = "US" Then
                ...
                If objAdd.SetLicenseString(ConfigurationManager.AppSettings("MelissaDataKey").ToString()) <> 1 Then
                    Return False
                End If
                ...
                If objAdd.Initialize(System.Configuration.ConfigurationManager.AppSettings("MelissaDataPath"), System.Configuration.ConfigurationManager.AppSettings("MelissaDataPath"), "") <> 0 Then
                    ...
                    objLogging.LogEvent(Server.MachineName & ": MelissaData is now expired!", Diagnostics.EventLogEntryType.Error & objAdd.GetInitializeErrorString)
        ...
        Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnUpdate.Click
            ...
            If UpdateAddress() Then
                ...
                Dim subjectofEmail = "Enroll Success Page Email Error."
                Dim dataDictionary As New Dictionary(Of String, String)
                dataDictionary.Add("ARVACT", lblAcctNo.Text)
                dataDictionary.Add("FIRST_NAME", lblFirstName.Text)
                dataDictionary.Add("LAST_NAME", lblLastName.Text)
                dataDictionary.Add("EMAIL_ADDRESS_", txtEmail.Text)
                dataDictionary.Add("FORM_SOURCE", "BGO")
                Dim result As Boolean = True 'SendDataToThirdpartyResponsys(dataDictionary, formDataTemplate, lblAcctNo.Text, subjectofEmail)

            End If

                'Me.ClientScript.RegisterStartupScript(Me.GetType(), "SubmitPost", "<script language='javascript'>document.frmPerformLogin.submit();</script>")
                Response.Redirect("Owner/enrollconfirm.aspx")
        ...
        Protected Function Login() As Boolean
            ...
            Try
                ...
                End If
                Console.WriteLine("**************************************")
                Console.WriteLine("Login Result = " + (sessionId <> Nothing))
                Console.WriteLine("**************************************")
                Console.WriteLine("Setting the Timeout to 1 hour")
            ...
            Catch e1 As Exception
                Console.WriteLine("Exception in login : " + e1.Message)
            End Try
            Return result
